...
def load_state_dict(model, loaded_state_dict, flownet=False, skip_modules=None,strict_loading=False):
    model_state_dict = model.state_dict()
    # if the state_dict comes from a model that was wrapped in a
    # DistributedDataParallel during serialization, unwrap it
    ...
    if dist.is_initialized():
        ...
        if strict_loading and mismatched_layers:
            print("Error: Strict loading is enabled, but the following layers have shape mismatches and were not loaded:")
            print(mismatched_layers)
            raise ValueError("Shape mismatches in strict loading. Model not loaded.")
        else:
            model.load_state_dict(model_state_dict, strict=False)