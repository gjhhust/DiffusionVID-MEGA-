...
class DiffusionDet(nn.Module):
    ...
    def _forward_test(self, imgs, infos, targets=None):
        ...
        if infos["ref_l"] or infos["ref_g"]:
            ...
            classes_t = torch.cat(classes_list_all, dim=0)
            classes_t = classes_t.view(-1, self.num_proposals, classes_t.shape[-1])
        ...
        batch = min(self.infer_batch, self.end_id - self.frame_id + 2) # 2  # len(self.feats)
        batch = max(batch,1)
